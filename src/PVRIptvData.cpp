  ...
  while(std::getline(stream, strLine))
  {
    ...
    if (StringUtils::Left(strLine, strlen(M3U_INFO_MARKER)) == M3U_INFO_MARKER)
    {
      ...
      if (iColon >= 0 && iComma >= 0 && iComma > iColon)
      {
        ...
        {
          tmpChannel.iCatchupDays = atoi(strCatchupDays.c_str());
        }
    ...
    else if (strLine[0] != '#')
    {
      ...
      if (!tmpChannel.strCatchupSource.empty())
      {
        channel.strCatchupSource = tmpChannel.strCatchupSource;
      }
      else if (!g_ArchiveConfig.GetArchiveUrlFormat().empty() && !strLine.empty())
      {
        channel.strCatchupSource = (strLine + g_ArchiveConfig.GetArchiveUrlFormat());
      }

      if (tmpChannel.iCatchupDays != 0)
      {
        channel.iCatchupDays = tmpChannel.iCatchupDays;
      }
      else
      {
        channel.iCatchupDays = g_ArchiveConfig.GetTimeshiftBuffer() / iptv::days_to_seconds(1);
      }
...
{
  return FindChannel((int)channel.iUniqueId, myChannel);
}
...
bool PVRIptvData::FindChannel(int uniqueId, PVRIptvChannel &myChannel)
{
  P8PLATFORM::CLockObject lock(m_mutex);
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    ...
    if (thisChannel.iUniqueId == uniqueId)
    {
      ...
      if (!GetLiveEPGTag(myChannel, myChannel.etEpgTag, true))
        myChannel.etEpgTag           = {0};
      return true;
    }
  }
...
std::string PVRIptvData::GetEpgTagUrl(const EPG_TAG *tag, PVRIptvChannel &myChannel)
{
  ...
  if (!tag && myChannel.etEpgTag.startTime > 0)
    return BuildEpgTagUrl(&myChannel.etEpgTag, myChannel);
  else if (!tag)
    return strUrl;
...
bool PVRIptvData::IsArchiveSupportedOnChannel(int uniqueId)
{
  ...
  if (!ret && FindChannel(uniqueId, channel))
