#include <ctime>
#include "CatchupUrlFormatter.h"
#include "../../client.h"

using namespace ADDON;
using namespace iptv;

std::string CatchupUrlFormatter::FormatCatchupUrl(const PVRIptvChannel &channel)
{
  ...
  XBMC->Log(LOG_DEBUG, "CatchupUrlFormatter::FormatCatchupUrl - \"%s\"", fmt.c_str());
  return fmt;
}

void CatchupUrlFormatter::FormatTime(const char ch, const struct tm *pTime, std::string &fmt)
{
  char str[] = { '{', ch, '}', 0 };
...
void CatchupUrlFormatter::FormatUtc(const char *str, time_t tTime, std::string &fmt)
{
  auto pos = fmt.find(str);
  if (pos != std::string::npos)
  {
